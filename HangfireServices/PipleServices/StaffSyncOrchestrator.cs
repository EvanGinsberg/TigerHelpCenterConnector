...
public class StaffSyncOrchestrator
{
    ...
    public StaffSyncOrchestrator(
        ...
        IMarkerService<ProcessingMarker> markerService,
        IConfiguration configuration)
    {
        ...
        _genericMarkerService = markerService;
        _markerFilePath = configuration["StaffSync:MarkerFilePath"] ??
    ...
    public async Task SynchronizeStaffAsync(PerformContext? context = null)
    {
        ...
        try
        {
            ...
            var marker = await _genericMarkerService.GetMarkerAsync(_markerFilePath);
            ...
            foreach (var change in changes.OrderBy(c => c.Timestamp))
            {
                ...
                await _genericMarkerService.UpdateMarkerAsync(marker, _markerFilePath);
            }
    ...
    private async Task ProcessStaffDetailsAsync(StaffDetails staffDetails)
    {
        try
        {
            ...
            var staffId = !string.IsNullOrEmpty(staffDetails.Uri) ?
                                    staffDetails.Uri.Split('/').Last() : "";
